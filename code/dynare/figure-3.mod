...
mkdir('output');
mkdir('output/data');
fid = fopen(sprintf('output/data/figure-3-liquidity-trap-duration-xip-%0.2f.csv', xip), 'w');
fprintf(fid, 'xip,shock,liquidity_trap_duration\n');
for i = 1:length(x)
	fprintf(fid, '%.15g,%.15g,%.15g\n', xip, x(i), liqduration(i));
end;
fclose(fid);
...
fid = fopen(sprintf('output/data/figure-3-multiplier-xip-%0.2f.csv', xip), 'w');
fprintf(fid, 'xip,shock,marginal_multiplier,average_multiplier\n');
for i = 1:length(liqmul_shock)
	fprintf(fid, '%.15g,%.15g,%.15g,%.15g\n', xip, liqmul_shock(i), liqmul_multiplier(i), avgmul(i));
end;
fclose(fid);

fid = fopen(sprintf('output/data/figure-3-government-debt-xip-%0.2f.csv', xip), 'w');
fprintf(fid, 'xip,shock,government_debt_multiplier\n');
for i = 1:length(pbalance)
	fprintf(fid, '%.15g,%.15g,%.15g\n', xip, x(i), pbalance(i));
end;
fclose(fid);
